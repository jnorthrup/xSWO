...
struct sparql_parser: public grammar<sparql_parser> {
	...
	struct definition {
		definition(sparql_parser const& /*self*/) {
			...
			WhereClause = !str_p("WHERE") >> GroupGraphPattern
					>> !BindingClause;
			...
			GraphTemplate = confix_p('{', !TriplesBlock >> *((str_p("GRAPH")
					>> VarOrIRIref >> GraphTemplate) >> !ch_p('.')
					>> !TriplesBlock), '}');;
			...
			ArgList = NIL | confix_p('(', Expression >> *(ch_p(',')
					>> Expression), ')');
			...
			RelationalExpression = NumericExpression >> !(ch_p('=')
					>> NumericExpression | str_p("!=") >> NumericExpression
					| ch_p('<') >> NumericExpression | ch_p('>')
					>> NumericExpression | str_p("<=") >> NumericExpression
					| str_p(">=") >> NumericExpression);
			...
			BuiltInCall = str_p("STR") >> confix_p('(', Expression, ')')
					| str_p("LANG") >> confix_p('(', Expression, ')') | str_p(
					"DATATYPE") >> confix_p('(', Expression, ')') | str_p(
					"BOUND") >> confix_p('(', Expression, ')') | str_p("isIRI")
					>> confix_p('(', Expression, ')') | str_p("isURI")
					>> confix_p('(', Expression, ')') | str_p("isBLANK")
					>> confix_p('(', Expression, ')') | str_p("isLITERAL")
					>> confix_p('(', Expression, ')')
					| str_p("LANGMATCHES") >> confix_p('(', Expression >> ch_p(
							',') >> Expression, ')')
					| str_p("sameTerm") >> confix_p('(', Expression
							>> ch_p(',') >> Expression, ')') | RegexExpression;
			RegexExpression = str_p("REGEX") >> ch_p('(') >> Expression
					>> ch_p(',') >> Expression >> !(ch_p(',') >> Expression)
					>> ch_p(')');
			...
			NCCHAR1p = alpha_p | range_p(0x00C0, 0x00D6) | range_p(0x00D8,
					0x00F6) | range_p(0x00F8, 0x02FF) | range_p(0x0370, 0x037D)
					| range_p(0x037F, 0x1FFF) | range_p(0x200C, 0x200D)
					| range_p(0x2070, 0x218F) | range_p(0x2C00, 0x2FEF)
					| range_p(0x3001, 0xD7FF) | range_p(0xF900, 0xFDCF)
					| range_p(0xFDF0, 0xFFFD) | range_p(0x10000, 0xEFFFF);
			NCCHAR1 = NCCHAR1p | '_';
			...
			NCNAME_PREFIX = *space_p >> NCCHAR1p >> !(*((NCCHAR | '.')
					- (NCCHAR >> ':')) >> NCCHAR);

			//NCNAME = NCCHAR1 >> !(*((NCCHAR | '.') - (NCCHAR >> ~eps_p(NCCHAR))) >> NCCHAR);
			NCNAME = (NCCHAR1 | digit_p) >> !(*((NCCHAR | '.') - (NCCHAR
					>> ~eps_p(NCCHAR))) >> NCCHAR);
		...
		rule<ScannerT> /*ANON,*/AdditiveExpression, ArgList, AskQuery,
				...
				LimitOffsetClauses, Load, Manage, MultiplicativeExpression, /*NCCHAR,
				 NCCHAR1, NCCHAR1p, NCNAME, NCNAME_PREFIX, NIL,*/
				NamedGraphClause, NumericExpression, NumericLiteral,
				NumericLiteralNegative, NumericLiteralPositive,
				NumericLiteralUnsigned, Object, ObjectList, OffsetClause,
				OptionalGraphPattern, OrderClause, OrderCondition, PNAME_LN,
				PNAME_NS, PrefixDecl, PrefixedName, PrimaryExpression,
				Prologue, PropertyList, PropertyListNotEmpty, Query,
				RDFLiteral, RegexExpression, RelationalExpression, Replace,
				STRING_LITERAL1, STRING_LITERAL2, STRING_LITERAL_LONG1,
				STRING_LITERAL_LONG2, SelectQuery, SolutionModifier,
				SourceSelector, String, TriplesBlock, TriplesNode,
				TriplesSameSubject, UnaryExpression, Update, VAR1, VAR2,
